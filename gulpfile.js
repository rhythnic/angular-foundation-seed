...
gulp.task('vendors', function () {
  ...
  return es.merge(
    ...
    bowerStream.pipe(g.filter(['**/*.js', '!**/angular.js', '!**/angular-ui-router.js'])).pipe(dist('js', 'vendors'))
  );
});
...
gulp.task('dist', ['vendors', 'styles-dist', 'scripts-dist'], function () {
  return gulp.src('./src/app/index.html')
    ...
    .pipe(g.replace('<!-- CDN -->', [
        ...
        '<script src="//cdnjs.cloudflare.com/ajax/libs/angular-ui-router/0.2.10/angular-ui-router.min.js"></script>'].join('')))
...
gulp.task('statics', g.serve({
  port: 3000,
  root: ['./.tmp', './.tmp/src/app', './src/app', './src/app/assets', './bower_components']
}));
 ...
 * Serve dist
 */
gulp.task('static-dist', g.serve({
  port: 3000,
  root: ['./dist', './src/app/assets']
}));

gulp.task('serve-dist', ['static-dist'], function() {
  g.livereload();
});
...
function cssFiles (opt) {
  return gulp.src(['./.tmp/css/styles/normalize.css', './.tmp/css/styles/foundation.css', './.tmp/css/**/*.css'], opt);
}
